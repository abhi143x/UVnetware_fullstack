      ...
      <svg width="140" height="70" viewBox="0 0 140 70">
        {Array.from({ length: 8 }).map((_, r) => {
          ...
          return Array.from({ length: count }).map((_, c) => {
            const angle =
              Math.PI * 0.25 + (c / (count - 1 || 1)) * Math.PI * 0.5;
            const rad = 20 + r * 6;
            return (
              <circle
                key={`${r}-${c}`}
                cx={70 + Math.cos(angle) * rad}
                cy={5 + rad - Math.sin(angle) * rad * 0.8}
                r={1.8}
                fill={d}
                opacity={0.6}
              />
    ...
    amphitheater: (
      <svg width="140" height="70" viewBox="0 0 140 70">
        {Array.from({ length: 7 }).map((_, r) => {
          ...
          return Array.from({ length: count }).map((_, c) => {
            const angle =
              Math.PI * 0.1 + (c / (count - 1 || 1)) * Math.PI * 0.8;
            const rad = 14 + r * 7;
            return (
              <circle
                key={`${r}-${c}`}
                cx={70 + Math.cos(angle) * rad * 0.9}
                cy={5 + rad - Math.sin(angle) * rad * 0.65}
                r={1.8}
                fill={d}
                opacity={0.6}
              />
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-[#0a0e14] via-[#0e1520] to-[#0a0e14] overflow-auto">
      {/* Ambient glows */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/3 w-125 h-125 bg-blue-500/5 rounded-full blur-[120px]" />
        <div className="absolute bottom-1/3 right-1/4 w-100 h-100 bg-cyan-500/5 rounded-full blur-[100px]" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-6 py-10">
        {/* Header */}
        <div className="text-center mb-10">
          <h1 className="text-4xl sm:text-5xl font-bold mb-2 bg-linear-to-r from-blue-400 via-cyan-300 to-blue-500 bg-clip-text text-transparent">
            Venue Templates
          </h1>
          <p className="text-sm text-[#6a7a8e] max-w-lg mx-auto">
            Pick a template to jump straight into a pre-built seating layout, or
            start from scratch.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            return (
              ...
              >
                ...
                <div className="px-4 pt-3 pb-1">
                  <div className="rounded-lg bg-[#0a0e14]/80 border border-white/5 p-2 flex items-center justify-center h-20 group-hover:border-[#587cb3]/20 transition-colors">
          ...
          >
            <div className="flex flex-col items-center justify-center h-full min-h-50 px-4 py-6">
