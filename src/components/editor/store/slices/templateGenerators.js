...
function createGroupedRowSeat(point, rowLetter, number, rowId) {
  return createGroupedSeat(point, rowLetter, number, ELEMENT_TYPES.ROW, rowId);
}

function createGroupedArcSeat(point, rowLetter, number, arcId) {
  return createGroupedSeat(point, rowLetter, number, ELEMENT_TYPES.ARC, arcId);
}
...
export function generateLargeArena() {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let s = 0; s < seatsInRow; s++) {
      ...
      seats.push(createGroupedArcSeat({ x, y }, rowLetter, s + 1, arcId));
    }
    rowIndex++;
  }
  texts.push({
