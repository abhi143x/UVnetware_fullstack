...
function PropertiesPanel() {
  ...
  const handleAddCategory = () => {
    ...
    if (!name) return
    const parsedPrice = newCategoryPrice === '' ? null : parseFloat(newCategoryPrice)
    addCategory({
      name,
          ...
          {hasSeatsSelected && (
            <>
              ...
              <div className="flex flex-col gap-4">
                ...
                {selectedSeat && !isMultipleSeats && (
                  <>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        type="text"
                        value={isMultipleSeats ? (getCommonSeatValue('label') || '') : (selectedSeat.label || '')}
                        onChange={(e) => handleSeatUpdate('label', e.target.value)}
                        placeholder={selectedSeat.row && selectedSeat.number ? `${selectedSeat.row}${selectedSeat.number}` : 'Auto'}
                    ...
                    <div className="flex gap-2">
                      <div className="flex flex-col gap-2 flex-1">
                        ...
                        <input
                          type="text"
                          value={isMultipleSeats ? (getCommonSeatValue('row') || '') : (selectedSeat.row || '')}
                      ...
                      <div className="flex flex-col gap-2 flex-1">
                        ...
                        <input
                          type="number"
                          value={isMultipleSeats ? (getCommonSeatValue('number') || '') : (selectedSeat.number || '')}
                          onChange={(e) => handleSeatUpdate('number', e.target.value ? parseInt(e.target.value) : null)}
                          placeholder="1"
                          className="rounded border border-white/10 bg-[#0e1319] px-3 py-1.5 text-white outline-none focus:border-[#587cb3]"
                        />
                      </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <select
                        value={isMultipleSeats ? (getCommonSeatValue('category') || '') : (selectedSeat.category || '')}
                        onChange={(e) => handleSeatUpdate('category', e.target.value || null)}
                        className="rounded border border-white/10 bg-[#0e1319] px-3 py-1.5 text-white outline-none focus:border-[#587cb3]"
                      >
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <select
                        value={isMultipleSeats ? (getCommonSeatValue('status') || 'available') : (selectedSeat.status || 'available')}
                        onChange={(e) => handleSeatUpdate('status', e.target.value)}
                        className="rounded border border-white/10 bg-[#0e1319] px-3 py-1.5 text-white outline-none focus:border-[#587cb3]"
                      >
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input
                        type="number"
                        step="0.01"
                        value={isMultipleSeats ? (getCommonSeatValue('price') || '') : (selectedSeat.price || '')}
                        onChange={(e) => handleSeatUpdate('price', e.target.value ? parseFloat(e.target.value) : null)}
                        placeholder="0.00"
                        className="rounded border border-white/10 bg-[#0e1319] px-3 py-1.5 text-white outline-none focus:border-[#587cb3]"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between rounded border border-white/10 bg-[#0e1319] px-3 py-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          disabled={selectedCategoryForInfo.price === null || selectedCategoryForInfo.price === undefined}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    value={isMultipleSeats ? (commonPrice === null ? '' : commonPrice) : (selectedSeat?.price ?? '')}
                    onChange={(e) => handleSeatUpdate('price', e.target.value ? parseFloat(e.target.value) : null)}
              ...
              <div className="flex flex-col gap-3">
                <h3 className="text-xs font-semibold tracking-wide text-white">Categories</h3>

                {categories.map((cat) => (
                  <div key={cat.id} className="rounded border border-white/10 bg-[#0e1319] p-3">
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <label className="text-[11px] text-gray-400 w-14">Price</label>
                      <input
                        type="number"
                        step="0.01"
                        value={cat.price ?? ''}
                        onChange={(e) => {
                          ...
                          updateCategory(cat.id, { price: Number.isNaN(parsed) ? null : parsed })
                        }}
