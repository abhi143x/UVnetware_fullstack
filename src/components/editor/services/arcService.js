...
import { createSeatGroupMetadata, DEFAULT_SEAT_RADIUS } from "./seatService";
...
const ARC_SEAT_SPACING_PADDING = 12;
...
export function normalizeArcAngle(value) {
  const parsed = normalizePositiveNumber(
    value,
    DEFAULT_ARC_TOOL_SETTINGS.arcAngle,
  );
...
export function calculateArcSeatSpacing(seatCount, arcAngle, radius) {
  ...
  return (
    2 * normalizedRadius * Math.sin(Math.max(stepRadians / 2, Number.EPSILON))
  );
}
...
export function buildArcSeatPlacements({
  ...
  const resolvedSeatType = seatType || SEAT_TYPES.CHAIR;
  const selectedSeatWidth =
    SEAT_TYPE_CONFIG[resolvedSeatType]?.defaultWidth ??
    SEAT_TYPE_CONFIG[SEAT_TYPES.CHAIR]?.defaultWidth ??
    DEFAULT_ARC_TOOL_SETTINGS.seatSpacing;
  const fixedSpacing = selectedSeatWidth + ARC_SEAT_SPACING_PADDING;
